...
# El arranque normal solo aplica migraciones y bootstraps no destructivos. Las importaciones,
# backfills y reinicios históricos son herramientas manuales: nunca deben ejecutarse durante
# un deploy porque la base puede contener operación real capturada después de la migración.
USER node
CMD ["sh", "-c", "npx --workspace server prisma migrate deploy && npm run seed -w server && npm run seed:operacion -w server && npm start"]
