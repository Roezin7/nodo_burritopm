...
function productoDTO(p: ProductoConRel) {
  return {
    ...
    markup_caja: p.tipo_operativo === 'proteina' ? 15 : num0(p.markup_caja),
  ...
  asyncHandler(async (req, res) => {
    ...
    const p = await prisma.$transaction(async (tx) => {
      const creado = await tx.products.create({ data: {
        ...
        markup_caja: b.tipo_operativo === 'proteina' ? 15 : (b.markup_caja ?? 0),
  ...
  asyncHandler(async (req, res) => {
    ...
    await prisma.$transaction(async (tx) => {
      await tx.products.update({ where: { id }, data: {
        ...
        markup_caja: (b.tipo_operativo ?? actual.tipo_operativo) === 'proteina' ? 15 : b.markup_caja,
