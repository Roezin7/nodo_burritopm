...
export default function HistorialPedidos({ pedidos, cargando, linea, semana, ubicacion, setUbicacion, ubicaciones, onPrint, onConfirmar, confirmando }: {
  ...
  return <div className="order-history">
    <section className="workspace-card history-toolbar history-toolbar--global">
      ...
      <div className="history-primary-actions"><button className="btn btn-secondary" disabled={cargando || confirmando} onClick={onConfirmar}>{confirmando ? 'Confirmando…' : 'Confirmar pedidos de la semana'}</button><button className="btn btn-primary" disabled={cargando || !filtrados.length} onClick={onPrint}>Imprimir por ruta</button></div>
    </section>
    {cargando ? <Spinner label="Cargando pedidos…" /> : <>
      <div className="metric-strip metric-strip--four"><div><span>Sucursales</span><strong>{new Set(filtrados.map((p) => p.ubicacion.id)).size}</strong></div><div><span>Pedidos confirmados</span><strong>{filtrados.length}</strong></div><div><span>Unidades de {linea}</span><strong>{unidades.toLocaleString('es-MX')}</strong></div><div><span>Importe de pedidos</span><strong>{usd(total)}</strong></div></div>
