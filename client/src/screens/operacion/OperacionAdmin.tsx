...
interface Factura { id: number; numero: string; version: number; empresa: string; ubicacion: string; linea: string; emitida_at: string; estado: string; total: number; pagado: number; lineas: { descripcion: string; cantidad: number; precio: number; importe: number }[] }
...
interface VistaPreviaCierre {
  ...
  facturas: { numero: string; empresa: string; ubicacion: string; linea: string; productos: number; unidades: number; total: number }[];
}
...
function Cierres({ cierres, semana, busy, setBusy, onDone, setError }: { cierres: Cierre[]; semana: SemanaSeleccionada; busy: boolean; setBusy: (v: boolean) => void; onDone: () => Promise<void>; setError: (v: string) => void }) {
  ...
  async function cerrar() { setBusy(true); setError(''); try { const r = await api<{ cajas_perdidas: number; productos_con_faltante: number }>('/cierre/cerrar', { method: 'POST', body: { fecha_cierre: semana.fin } }); setVistaPrevia(null); await onDone(); toast.ok(r.cajas_perdidas > 0 ? `Semana cerrada · ${r.cajas_perdidas.toLocaleString('es-MX')} de faltante teórico registrados en Incidencias. Valida con conteo físico si necesitas investigar una merma.` : 'Semana cerrada correctamente.'); } catch (e) { setVistaPrevia(null); setError(e instanceof ApiError ? e.message : 'No se pudo cerrar la semana.'); } finally { setBusy(false); } }
  ...
  return <div className="operation-stack">
    ...
    <div className="week-list">{cierreSeleccionado ? [cierreSeleccionado].map((s) => <section className="week-card" key={s.id}><header><div><span className={`status-dot status-dot--${s.estado}`} /> <strong>Semana {s.semana} · {s.anio}</strong><p>{s.inicia_at} al {s.termina_at}</p></div><div className="week-balance"><span>{semanaEnCurso ? 'Semana en curso' : s.estado === 'cerrada' ? 'Balance' : 'Balance actual · vista previa'}</span><strong>{semanaEnCurso ? panoramaEnCurso ? usd(panoramaEnCurso.cartera.balance_neto) : panoramaEnCurso === undefined ? 'Cargando…' : 'No disponible' : s.estado === 'cerrada' ? usd(s.balance_neto) : balanceVivo == null ? 'Calculando…' : usd(balanceVivo)}</strong></div></header>
      ...
      <CollapsibleSection title="Facturas" count={s.facturas.length}><div className="invoice-list"><div className="invoice-row invoice-row--head"><span>Factura</span><span>Empresa / ubicación</span><span>Emitida</span><span>Estado</span><span>Total</span><span /></div>{s.facturas.map((f) => <div className="invoice-row" key={f.id}><button className="invoice-number" onClick={() => setFactura(f)}>{f.numero}<small>v{f.version} · {f.linea}</small></button><span data-label="Ubicación"><strong>{f.ubicacion}</strong><small>{f.empresa}</small></span><span data-label="Emitida">{f.emitida_at}</span><span data-label="Estado"><span className="chip chip--info">Ciclo de 3 semanas</span></span><span data-label="Total"><strong>{usd(f.total)}</strong></span><span /></div>)}</div></CollapsibleSection>
    ...
    {vistaPrevia && <Modal className="close-preview-modal" ariaLabelledBy="close-preview-title" closeOnBackdrop={!busy} closeOnEscape={!busy} onClose={() => setVistaPrevia(null)}>
      ...
      <details className="operation-help"><summary>Cómo se calcula el inventario</summary><p>Saldo inicial + compras + producción − ventas. El conteo físico es opcional: sólo hazlo cuando quieras validar una diferencia o investigar una posible merma. Los saldos negativos se reportan como faltante teórico, no como merma confirmada, y se valúan en cero.</p></details>
      ...
      {vistaPrevia.cajas_perdidas > 0 && <p className="notice notice--warning"><strong>{vistaPrevia.cajas_perdidas.toLocaleString('es-MX')} de faltante teórico</strong> en {vistaPrevia.productos_con_faltante} {vistaPrevia.productos_con_faltante === 1 ? 'producto' : 'productos'}. Se valuarán como cero y se crearán incidencias; no bloquean el cierre. Un conteo físico puede confirmar o descartar una merma.</p>}
      <details className="close-preview-support">
        <summary><span><strong>Ver información de soporte</strong><small>Desgloses para consulta; no agregan importes al resumen del cierre</small></span><Icono name="chevron" /></summary>
        <div className="close-preview-support-body">
      <CollapsibleSection title="Venta por ubicación y línea" count={vistaPrevia.ventas.por_ubicacion.length}>
      ...
      <CollapsibleSection title="Facturas por generar" count={vistaPrevia.facturas.length}><div className="invoice-list close-preview-invoices"><div className="invoice-row invoice-row--head"><span>Factura</span><span>Empresa / ubicación</span><span>Periodo</span><span>Línea</span><span>Total</span></div>{vistaPrevia.facturas.map((f) => <div className="invoice-row" key={f.numero}><span className="invoice-number">{f.numero}<small>{f.productos} productos · {f.unidades.toLocaleString('es-MX')} unidades</small></span><span data-label="Ubicación"><strong>{f.ubicacion}</strong><small>{f.empresa}</small></span><span data-label="Periodo">Semana en cierre</span><span data-label="Línea"><span className="chip">{f.linea}</span></span><span data-label="Total"><strong>{usd(f.total)}</strong></span></div>)}</div></CollapsibleSection>
        </div>
      </details>
      <div className="close-preview-actions"><button className="btn btn-secondary" disabled={busy} onClick={() => setVistaPrevia(null)}>Seguir revisando</button><button className="btn btn-primary" disabled={busy} onClick={() => void cerrar()}>{busy ? 'Cerrando…' : 'Confirmar cierre'}</button></div>
    </Modal>}
    {factura && <Modal className="invoice-print" ariaLabelledBy="invoice-preview-title" onClose={() => setFactura(null)}><div className="card-head"><div><span className="eyebrow">Factura</span><strong id="invoice-preview-title">M&amp;G Management and Logistics Inc.</strong></div><button className="icon-btn" aria-label="Cerrar" onClick={() => setFactura(null)}><Icono name="x" /></button></div><h2>{factura.ubicacion}</h2><p>{factura.empresa} · {factura.numero} · emitida {factura.emitida_at}</p><div className="invoice-detail">{factura.lineas.map((l, i) => <div key={i}><span><strong>{l.descripcion}</strong><small>{l.cantidad} × {usd(l.precio)}</small></span><strong>{usd(l.importe)}</strong></div>)}</div><div className="invoice-grand-total"><span>Total</span><strong>{usd(factura.total)}</strong></div><button className="btn btn-primary btn-block" onClick={() => window.print()}>Imprimir / guardar PDF</button></Modal>}
  </div>;
}
