...
function EstadoSalidaChip({ estado }: { estado: string }) {
  const completada = ESTADOS_COMPLETADOS.includes(estado);
  const label = estado === 'cerrada_con_incidencias' ? 'Despachada c/ faltantes' : completada ? 'Despachada' : estado === 'cancelada' ? 'Cancelada' : 'Programada';
  const cls = estado === 'cancelada' ? 'chip--danger' : completada ? (estado === 'cerrada_con_incidencias' ? 'chip--warn' : 'chip--ok') : 'chip--info';
  return <span className={`chip chip-estado ${cls}`}>{label}</span>;
}

export default function Entregas({ integrado = false, semana = crearSemana() }: { integrado?: boolean; semana?: SemanaSeleccionada; onChange?: (inicio: string) => void }) {
  ...
  const salidasPor = (fecha: string, linea: LineaOperacion) => {
    const salidas = lista
      ...
      .filter((distribucion) => mostrarCompletadas || !ESTADOS_COMPLETADOS.includes(distribucion.estado))
      .sort((a, b) => a.id - b.id);
            ...
            {(['carne', 'desechables'] as const).map((linea) => {
              ...
              return <div className={`dispatch-day-cell dispatch-day-cell--${linea}`} key={linea}>
                {salidas.length ? salidas.map((salida) => <button className="dispatch-day-card" key={salida.id} onClick={() => void abrir(salida.ids)}>
                  <div><strong>{salida.total_lineas} partidas</strong><span>Preparar salida</span></div><div><EstadoSalidaChip estado={salida.estado} /><b><Icono name="chevron" /></b></div>
