    ...
    <div className={integrado ? 'order-page order-embedded' : 'page order-page'}>
      ...
      {vista === 'captura' ? capturaSemanal ? <CapturaSemanalPedidos catalogo={catalogo} linea={linea} semana={semana} ubicaciones={ubicaciones} semanaCerrada={semanaCerrada} onActualizado={() => setRefrescoHistorial((n) => n + 1)} /> : <div className={`order-workspace order-workspace--guided order-workspace--${pasoIndividual}`}>
        <section className="order-capture order-capture--guided">
          ...
          {!semanaCerrada && editable && admin && estado && !['borrador', 'confirmado'].includes(estado) && <p className="context-note">Corrección: actualiza pedido, despacho e inventario.</p>}

          {pasoIndividual === 'captura' ? <section className="workspace-card guided-products" aria-labelledby="productos-title">
            <header className="guided-products__head"><div><span className="eyebrow">Captura</span><h3 id="productos-title">¿Qué necesitas?</h3><p>Captura cantidades.</p></div><span className="guided-products__count">{seleccionados.length} seleccionados</span></header>
            ...
            </article>; })}{!visibles.length && <div className="empty-state"><strong>{filtroProductos === 'seleccionados' ? 'Sin productos' : 'Sin resultados'}</strong><span>{filtroProductos === 'seleccionados' ? 'Selecciona Todos para verlos.' : 'Cambia la búsqueda o el filtro.'}</span></div>}</div>}
          </section> : <section className="workspace-card order-review" aria-labelledby="revision-title">
            <header><div><span className="eyebrow">Revisión</span><h3 id="revision-title">Revisa tu pedido</h3><p>Verifica cantidades.</p></div><button type="button" className="btn btn-secondary" onClick={() => setPasoIndividual('captura')}>Seguir editando</button></header>
            ...
            <p className="order-review__explanation">Al confirmar, el pedido queda registrado.</p>
          </section>}
        </section>
